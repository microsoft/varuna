...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None):
        # print('BertEmbeddings.weight = ', self.word_embeddings.weight)
        ...
        #     gpu_rng_states = torch.cuda.get_rng_state(torch.cuda.current_device())
        #     print('modeling.py: srngs: cpu = ', sha1(cpu_rng_state.cpu().numpy()).hexdigest(), '  gpu = ', sha1(gpu_rng_states.cpu().numpy()).hexdigest())
        embeddings = self.dropout(embeddings)
        # if torch.distributed.get_rank()==0:
        #     cpu_rng_state = torch.get_rng_state()
        #     gpu_rng_states: Optional[ByteTensor]
        #     gpu_rng_states = torch.cuda.get_rng_state(torch.cuda.current_device())
        #     print('post_modeling.py: srngs: cpu = ', sha1(cpu_rng_state.cpu().numpy()).hexdigest(), '  gpu = ', sha1(gpu_rng_states.cpu().numpy()).hexdigest())
        return embeddings
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.cutpoints = nn.ModuleList([CutPoint() for i in range(config.num_hidden_layers - 1)])
