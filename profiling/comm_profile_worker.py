...
prev_size = torch.Size([1,1024,1600])
next_size = torch.Size([1,1024, 1600])
...
def sender(send_rank, mBS, send_times):
    ...
    for _ in range(chunks):
        output_acts = torch.ones(send_shape, dtype=dtype, device=local_rank)
        start_time = time.time()
        handle = dist.isend(output_acts.cpu(), dst=send_rank)
...
out_file = "/home/varuna/gpt2-blob/perf_analysis_1.5b/comm_times_sync_2way_clstrd_{}".format(rank)
