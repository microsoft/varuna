...
# out_filename = "/home/varuna/gpt2-blob/single_gpu_perf/all_red_rep_{}_ring_{}_clstrd_{}.txt".format(replicas, ring_size, rank)
...
print("dist init done!",flush=True)
...
# small dummy warmup
print("doing compute for warmup",flush=True)
m = torch.nn.Linear(16384, 16384)
m.cuda()
for i in range(200):
    inputs = torch.randn(1024, 16384)
    inputs = inputs.cuda()
    output = m(inputs)
...
if embed_group is not None:
    out_file = open("/home/varuna/gpt2-blob/single_gpu_perf/embed_all_red_{}".format(rank),"a")
    avg_time = 0.0
    for _ in range(5):
        start_time = time.time()
