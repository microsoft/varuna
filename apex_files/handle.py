...
def scale_loss(loss,
    ...
    """
    # if sync_step:
    #     return sync_loss_scale(optimizers)
    ...
    if last_partition:
        ...
        # print(torch.distributed.get_rank(), ': handle.py: first gpu. not scaling loss')
        yield loss.float()
    ...
    if delay_unscale:
        ...
        # if not isinstance(optimizers, FP16_Optimizer_for_fused):
            ...
            should_skip = False if delay_overflow_check else loss_scaler.update_scale_custom(last_microbatch)
            if should_skip:
                for optimizer in optimizers:
...
# myedits
@contextlib.contextmanager
def sync_loss_scale(optimizers,
               loss_id=0,
               model=None,
               delay_unscale=False,
               delay_overflow_check=False):
