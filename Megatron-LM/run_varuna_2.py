...
def read_profile(args, stage_num):
    prefix = "gpt2_345m_profile_fp16-"
    profile = [dict()]
    with open(prefix + str(stage_num) + ".csv", "r") as f:
        f.readline()
        for line in f:
            fields = line.split(",")
            mbs = int(fields[0])
            max_mem = int(fields[3])
            acts_size = int(fields[5])
            assert len(profile) == mbs, "Profile reading error!"
            profile.append(dict({"mem": max_mem, "acts":acts_size}))
    return profile

def calculate_memory(args, profiles):
    profile_1, profile_2, profile_last = profiles
...
if __name__ == "__main__":
    ...
    profile_1 = read_profile(args, 1)
    profile_2 = read_profile(args, 2)
    profile_last = read_profile(args, args.total_num_stages)

    loop_count = 0
    ppl_depths = []
    for i in range(1,args.total_num_stages+1):
        if args.total_num_stages % i == 0:
            ppl_depths.append(i)
