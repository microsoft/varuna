...
and used for :class:`Varuna` initialisation.

Sample inputs (with any batch size) also need to be passed for this automatic partitioning. These inputs
are used to profile the model's computation graph and should be passed a a dictionary of keywords to args.
